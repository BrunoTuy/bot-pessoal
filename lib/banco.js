...
module.exports = async ({ MONGO_URI, MONGO_DB }) => {
  ...
  const update = async ({ colecao, registro, id, set }) => {
    try {
      ...
      if (modifiedCount < 1 && registro.cartaoId && registro.cartaoId.length == 24) {
        return await update({ colecao, registro: {
          ...registro,
          cartaoId: new ObjectId(registro.cartaoId)
        }, id, set })
      }
  ...
  const remove = async ({ colecao, registro }) => {
    ...
    try {
      const collection = db.collection(colecao);
      const { deletedCount } = await collection.deleteOne(registro);

      if (deletedCount < 1 && registro.cartaoId && registro.cartaoId.length == 24) {
        return await remove({ colecao, registro: {
          ...registro,
          cartaoId: new ObjectId(registro.cartaoId)
        }})
      }
  ...
  const get = async ({ colecao, registro }) => {
    try {
      ...
      if (!i && registro.cartaoId && registro.cartaoId.length == 24) {
        return await get({ colecao, registro: {
          ...registro,
          cartaoId: new ObjectId(registro.cartaoId)
        }})
      }
