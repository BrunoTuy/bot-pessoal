...
const exec = async ({ callback, lib, libLocal, parametros }) => {
  const listaPadrao = (lista, linhas) => {
    ...
    lista.forEach(({ encerrado, datas, cotas, corretora, valor, valorSaida, rendimento }) => {
  ...
  if (!data) {
    ...
    const linhas = [doc.id];
    const { lista, tipo } = data;

    if (tipo === 'FII') {
      listaFII(lista, linhas);
    } else {
      listaPadrao(lista, linhas);
    }
